...
export default function ReturnPolicyPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Use refund-policy content for return policy as well
        const p = await fetchLegal('refund-policy');
        if (!mounted) return;
        setPage(p);
      } catch (err) {
        if (!mounted) return;
        setError(err.response?.data?.message || 'Unable to load return policy.');
      } finally {
    ...
    <div className="container section">
      <div className="section-header">
        <div>
          ...
          <h1 className="section-title">{page?.title || 'Return Policy'}</h1>
        </div>
      </div>
